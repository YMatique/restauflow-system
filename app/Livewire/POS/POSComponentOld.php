...
// Define se a view principal é 'tables' ou 'products'
    ...
    public function handleTableSelection($tableId): void
    {
        // CORREÇÃO: Aceitar tanto int quanto array (compatibilidade)
        if (is_array($tableId)) {
            $tableId = $tableId[0] ?? $tableId['tableId'] ?? null;
        }
    ...
    public function render()
    {
        ...
        logger("POSComponent render - currentView: {$this->currentView}, currentTable: " . 
               ($this->currentTable ? $this->currentTable->name : 'null'));
